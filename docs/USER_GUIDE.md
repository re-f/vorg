...
#### 标题操作功能
1. **升级子树**：`Ctrl+C Ctrl+Shift+,` (类似 Emacs `C-c C-<`)
   - 将光标放在标题行上
   - 执行升级操作，整个子树的标题级别减少 1
   - 1 级标题不能再升级
   
2. **降级子树**：`Ctrl+C Ctrl+Shift+.` (类似 Emacs `C-c C->`)
   - 将光标放在标题行上
   - 执行降级操作，整个子树的标题级别增加 1

3. **CodeLens 按钮**（需启用 `vorg.showCodeLens` 配置）
   - 每个标题行上方会显示操作按钮
