...
### 插入/切换强调标记（`vorg.emphasize`）

对应 Emacs `org-emphasize`（`C-c C-x C-f`）：选中一段文本后执行该命令，可以快速包裹或去除以上六种标记之一。

- 快捷键：`Ctrl+C Ctrl+X Ctrl+F`
- 选中文本后触发：弹出选择列表，选择粗体/斜体/下划线/删除线/逐字/代码之一
  - 若选中文本尚未被该标记包裹，会自动包裹（并把首尾空白挪到标记外面，因为 Org 标记内容首尾不能是空白）
  - 若选中文本已经被该标记完整包裹，会自动去除标记（切换/toggle 行为）
- 未选中文本时触发：在光标处插入一对标记字符，并将光标定位到中间

### 隐藏强调标记字符（`vorg.hideEmphasisMarkers`）

对应 Emacs `org-hide-emphasis-markers`。默认关闭（标记字符始终可见）。在设置中开启：

```json
{
  "vorg.hideEmphasisMarkers": true
}
```

开启后，编辑器中 `* / _ = ~ +` 这几个标记字符会被隐藏，只显示强调后的文本样式（粗体/斜体/下划线/删除线/等宽底色）。当光标移动到某个已隐藏标记的范围内时，该处的标记字符会临时重新显示，方便正常编辑；光标移开后自动恢复隐藏。

> 该开关同时控制"是否额外渲染强调样式装饰"：关闭时不做额外装饰渲染，仅依赖 TextMate 语法高亮（`markup.*.org` scope）自带的着色；开启隐藏标记后才需要额外渲染样式，否则标记消失后无法看出文本曾被强调。VOrg 不单独提供 Emacs `org-fontify-emphasized-text` 对应的开关。
