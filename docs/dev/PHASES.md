# VOrg 开发阶段 (Phase) 记录

本文档记录 VOrg 扩展的主要开发阶段和里程碑。

## Phase 0: 结构化存储基础设施 (进行中)

**目标**: 实现基于 SQLite 的结构化存储系统,为 Agenda、搜索、反向链接等功能提供高性能数据层。

**时间**: 2026-01-28 开始,预计 3-4 周

**参考架构**: org-roam (Emacs Org-mode 知识管理系统)

### Stage 1: 数据库层基础 (3-4 天) ✅ 已完成

#### ✅ Stage 1.1: 项目依赖配置 (已完成 2026-01-28)
- 技术选型: better-sqlite3 vs sql.js
- 安装 better-sqlite3@12.6.2
- 配置 webpack 处理 native 模块
- 文档: `docs/TECHNICAL_DECISIONS.md`

#### ✅ Stage 1.2: 数据模型定义 (已完成 2026-01-28)
- TypeScript 类型定义 (250+ 行)
- 单元测试: 17/17 通过

#### ✅ Stage 1.3: 数据库 Schema 设计 (已完成 2026-01-28)
- SQL Schema (200+ 行): 5 表, 15+ 索引, 3 视图
- SchemaManager 类 (220+ 行)
- 单元测试: 17/17 通过

#### ✅ Stage 1.4: 数据库连接管理 (已完成 2026-01-28)
- DatabaseConnection 单例模式 (280+ 行)
- 事务支持、备份恢复
- 单元测试: 19/19 通过

#### ✅ Stage 1.5: 基础 CRUD 操作 (已完成 2026-01-28)
- FileRepository 类 (180+ 行)
- 完整 CRUD 操作
- 单元测试: 18/18 通过

**Stage 1 总结**:
- 代码: ~1200+ 行
- 测试: 71/71 通过
- 完成日期: 2026-01-28
- Commits: 3 个

### Stage 2: 文件解析与索引 (4-5 天) 🔄 进行中
- ✅ HeadingRepository 实现 (19 tests)
- ✅ LinkRepository 实现 (17 tests)
- ✅ UniorgAstExtractor 实现 (22 tests)
- ✅ FileIndexer 实现
- ✅ WorkspaceIndexer 实现 (3 verified tests)
- ✅ Stage 2.6: Heading & Link Schema 调整 (140 verified tests)
- 性能优化

### Stage 3: 增量更新机制 (3-4 天) ✅ 已完成
- ✅ 文件监听器 (`WorkspaceWatcher`, 支持 `.org` 和 `.org_archive`)
- ✅ 变更检测与防抖逻辑
- ✅ 差异更新与删除同步
- ✅ 错误恢复与日志记录

### Stage 4: 现代化查询系统 (Modernized Query System) 🟡
**目标**：构建一个基于节点的、多维度的检索与视图系统，远超简单的文件搜索。

- **4.1 嵌入式查询块 (Embedded Query Blocks)** ✅
  - [x] 实现 `#+BEGIN_QUERY` 块的识别与渲染
  - [x] 构建 `QueryService` 与增强型 `HeadingRepository`
  - [x] 支持多维过滤 (TODO, Priority, Tags, Pinyin Search)
  - [x] 实现预览中的交互式跳转
- **4.2 属性过滤与聚合 (Property Filtering)** 📅
- ⏳ Stage 4.3: 结果丰富化与摘要提取 (Result Hydration)
- ⏳ Agenda 查询与日期过滤
- ⏳ 链接与反向链接查询 (Backlinks)

### Stage 5: 集成与测试 (2-3 天)
- 服务层封装
- VS Code 集成
- 命令实现
- 端到端测试

**验收标准**:
- ✅ 所有单元测试通过 (140/140 tests, 包含 Parser、Database 和 Indexer 测试)
- ✅ 所有集成测试通过 (36/36 tests, 手工验证)
- ⏳ E2E 测试通过
- ⏳ 性能指标达标:
  - 索引 100 文件 < 5 秒
  - 单次查询 < 50ms
  - 增量更新 < 100ms

**进度更新**:
- ✅ 2.4 FileIndexer 实现 (单文件索引，支持事务更新)
- ✅ 2.5 WorkspaceIndexer 实现 (多文件批量处理)
- ✅ 2.6 Schema 优化: 支持无 ID 标题的复合主键引用与反向链接
---

## Phase 1: 编辑增强 (计划中)

**目标**: 实现高级编辑功能,提升日常使用效率

**预计时间**: 2-3 周

### 功能列表
- 列表项移动/缩进 (高优先级)
- 子树移动 (`org-metaup/down`) (中优先级)
- Refile 功能 (中优先级)

---

## Phase 2: TODO 与时间管理 (计划中)

**目标**: 完整的任务管理功能

**预计时间**: 2-3 周

### 功能列表
- 优先级与标签系统
- SCHEDULED/DEADLINE 日期
- 标签搜索
- 重复任务 (低优先级)

---

## Phase 3: 现代化查询系统与性能枢纽 (进行中)

**目标**: 实现类似于 org-ql 的多维动态查询系统,将 Org 文件转变为个人仪表盘。

### 三大核心场景
- ✅ **B. 嵌入式查询块 (Embedded Query Blocks)**: 在文档中嵌入动态结果 (进行中)。
- ⏳ **A. 智能全局搜索 (Quick Query)**: 增强型 QuickPick,支持实时预览与侧边栏渲染。
- ⏳ **C. 侧边栏“透视”导航 (Perspectives)**: 侧边栏保存的动态视图与分组展示。

### 功能增强
- ⏳ Property 智能补全与键值过滤。
- ⏳ 动态 Webview 渲染组件 (用于嵌入式查询结果)。

## Phase 4: 现代化 Agenda 视图 (计划中)

**目标**: 打造现代化的任务总览界面

**预计时间**: 3-4 周

### 功能列表
- Webview UI 设计
- 多视图模式 (日历、看板、时间线、列表)
- 拖拽交互
- 实时过滤

---

## Phase 5+: 其他功能 (计划中)

### 中优先级
- 快速捕获
- 归档功能

### 低优先级
- Markdown 导出
- 表格编辑
- Sparse Tree 搜索
- 其他高级功能

---

## 里程碑

- **v0.1.0**: Phase 0 完成 - 结构化存储就绪
- **v0.2.0**: Phase 2 完成 - 任务管理完善
- **v0.3.0**: Phase 3 完成 - Agenda 上线
- **v1.0.0**: 所有高/中优先级功能完成 (目标: 2026 年中)

---

## 更新日志

- 2026-01-28: Phase 0 启动
  - Stage 1.1-1.3 完成 (依赖、类型、Schema)
  - Stage 1.4 完成 (数据库连接管理)
  - Stage 1.5 完成 (File Repository)
  - **Stage 1 完成** ✅ (71/71 测试通过)
- 2026-01-29: Phase 0 架构升级、增量索引与 Phase 2 核心编辑功能
  - Stage 2.1-2.3 完成 (Repositories & Extractor)
  - Stage 2.4-2.5 完成 (File & Workspace Indexer)
  - **Stage 2.6 完成** ✅ 实现健壮的复合主键架构，支持所有标题的反向链接
  - **Stage 2 核心功能完成** ✅ (140/140 测试通过)
  - **Stage 3 完成** ✅ 实现 `WorkspaceWatcher` 与 `IncrementalUpdateService`，支持 `.org` 和 `.org_archive` 实时同步
  - **Phase 2 核心任务管理功能完成** ✅ 实现优先级循环、标签编辑、SCHEDULED/DEADLINE 设置
  - **集成测试通过** ✅ 57/57 项集成测试通过，涵盖复杂正文触发场景
  - **缓存架构清理与持久化搜索 API 完成** ✅ 彻底移除 `OrgSymbolIndexService` 的内存缓存与冗余 FileWatcher，实现基于 SQLite 的拼音搜索与持久化索引，Stage 4 查询 API 基础架构就绪
- 2026-01-30: 数据库 Schema 整合与工作区符号导航优化
  - **Schema 统一化** ✅ 将数据库版本回退并整合至 Version 1，拼音字段成为初始安装的一部分，删除冗余迁移逻辑。
  - **存储路径迁移** ✅ 数据库文件 `.vorg.db` 现在优先存储在工作区根目录下，方便用户管理。
  - **跳转精度修复** ✅ 实现 `OffsetMapper`，将 AST 偏移量准确转换为行号，修复 `Cmd+T` 搜索无法精准跳转的问题。
  - **测试全覆盖** ✅ 单元测试增加至 386 项，集成测试增加至 59 项，所有测试全部通过。
- 2026-01-30: 现代化查询系统启动 (Stage 4.1)
  - **嵌入式查询块完成** ✅ 实现 `#+BEGIN_QUERY` 块的识别、动态执行与渲染。
  - **多维查询引擎** ✅ `QueryService` 支持 TODO、优先级、标签、关键词 (含拼音) 的组合过滤。
  - **交互跳转支持** ✅ 预览中的查询结果可点击并立即跳转至源码对应行。
  - **单元测试增加** ✅ 新增 `findByCriteria.test.ts` 专项验证查询逻辑。
