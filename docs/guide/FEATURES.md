...
### 核心功能
- **插入/切换强调标记**：类似 Emacs `org-emphasize` 的完整实现，支持粗体、斜体、下划线、删除线、逐字、代码六种标记
- **隐藏强调标记字符**：类似 Emacs `org-hide-emphasis-markers`，可选隐藏 `* / _ = ~ +` 标记字符，只显示强调后的样式
- **光标感知显示**：标记隐藏时，光标移入标记范围会临时显示标记字符，方便编辑

### 使用方法

#### 插入/切换标记
1. **快捷键**：`Ctrl+C Ctrl+X Ctrl+F` (类似 Emacs `C-c C-x C-f`)
2. **命令面板**：`VOrg: Emphasize`
3. **操作流程**：
   - 选中一段文本，执行命令
   - 在弹出的列表中选择粗体/斜体/下划线/删除线/逐字/代码之一
   - 若选中文本尚未被该标记包裹，会自动包裹；若已完整包裹，会自动去除（切换行为）
   - 未选中文本时，会在光标处插入一对标记字符并将光标定位到中间
...
```
开启后标记字符会被隐藏，只显示强调后的文本样式；光标移入某个标记范围内时会临时显示该处的标记字符。
...
### 链接插入与导航（统一逻辑）
- **命令插入**：`Ctrl+C Ctrl+L` → Heading，与补全一致——跨文件选标题、生成 `[[id:UUID][标题]]`、自动补 ID
- **跟随链接**：`Ctrl+C Ctrl+O` 支持 `id:`、标题文本、`*标题`、`file:path` 及 `file:path::*标题` / `file:path::#id`
- **Ctrl+Click / F12**：与跟随链接共用解析服务；`id:` 链接通过延迟解析显示可点击下划线
