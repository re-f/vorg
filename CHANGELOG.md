...
- **Org Refile（子树移动）**：支持将子树移动到同一文件或跨文件的指定目标标题下。
  - **使用方法**：将光标放在要移动的标题行上，按 `Ctrl+C Ctrl+W`（Mac 为 `Ctrl+C Ctrl+W`，Windows/Linux 同），调出 Quick Pick 选择目标位置。跨文件目标会在描述中显示文件路径。

### 🔧 改进优化

- **拼音搜索增强**：工作区符号搜索现在支持文件名路径匹配，提升跨文件跳转体验。
- **跨文件搜索优化**：Quick Pick 描述中包含文件路径，便于在多文件项目中准确选择目标。

### 🐛 Bug 修复

- **集成测试修复**：修复了 9 个因索引时机问题导致的集成测试失败。
