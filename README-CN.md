...
- **跨文件 ID 补全**：输入 `[[` 或 `[[id:` 即可检索全工作区标题并插入稳定的 `[[id:UUID][标题]]`；目标标题无 ID 时自动写入。
- **链接插入统一**：`Ctrl+C Ctrl+L` → Heading 与 `[[` 补全共用同一套跨文件 id 链接逻辑。
- **链接导航统一**：`Ctrl+C Ctrl+O`、Ctrl+Click / F12 共用同一解析器，支持 `file:path::*标题` 与 `file:path::#id` 锚点。
