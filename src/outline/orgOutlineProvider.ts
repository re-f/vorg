...
export class OrgOutlineProvider implements vscode.DocumentSymbolProvider {
  ...
  provideDocumentSymbols(
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 处理文档元数据（#+TITLE, #+AUTHOR, 等）- 支持大小写
      const metadataMatch = line.match(/^#\+([A-Za-z_]+):\s*(.+)$/);
      if (metadataMatch) {
        const key = metadataMatch[1].toUpperCase(); // 转换为大写进行比较
      ...
      if (headingInfo.level > 0) {
        ...
        for (let j = i + 1; j < lines.length; j++) {
          const nextLineHeadingInfo = HeadingParser.parseHeading(lines[j], false, todoKeywords);
