...
describe('VOrgQLParser (S-Expression)', () => {
    it('应该解析基础的 and 逻辑', () => {
        ...
        assert.strictEqual(arg1.args[0], 'TODO');

        const arg2 = ast.args[1] as any;
        assert.strictEqual(arg2.type, 'priority');
        assert.strictEqual(arg2.args[0], 'A');
    });

    it('应该处理操作符别名 (p -> priority, # -> tag)', () => {
    ...
    it('应该解析 group-by 嵌套结构', () => {
        ...
        assert.strictEqual(subQuery.args[0], 'NEXT');
    });
    ...
    it('应该处理纯文本搜索', () => {
        ...
        const ast = VOrgQLParser.parse(query);
        assert.strictEqual(ast.type, 'text');
        assert.strictEqual(ast.args[0], 'database search');
    });
});
